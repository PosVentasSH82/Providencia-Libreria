  ...
  cashClosings: JSON.parse(localStorage.getItem('libreria_sh82_cash_closings') || '[]'),
  cashSession: JSON.parse(localStorage.getItem('libreria_sh82_cash_session') || 'null'),
  users: JSON.parse(localStorage.getItem('libreria_sh82_users') || '[]'),
  currentUser: JSON.parse(localStorage.getItem('libreria_sh82_current_user') || 'null'),
  settings: JSON.parse(localStorage.getItem('libreria_sh82_settings') || '{"title1":"Librería SH82","title2":"Pantalla principal","posTitle":"POS Librería SH82","posSubtitle":"Ventas, productos, deudas, cierres y resumen diario.","logoDataUrl":"","accentColor":"#1f7a5c","bgColor":"#f7f7fb","cardColor":"#ffffff","logoSize":120,"title1Size":32,"title2Size":16,"title1Font":"Inter, system-ui, sans-serif","title2Font":"Inter, system-ui, sans-serif","title1Color":"#1d2530","title2Color":"#6f7a86","posLogoSize":56,"ordersEnabled":true}'),
  categories: JSON.parse(localStorage.getItem('libreria_sh82_categories') || '[]'),
  people: JSON.parse(localStorage.getItem('libreria_sh82_people') || '[]'),
  stockConfig: JSON.parse(localStorage.getItem('libreria_sh82_stock_config') || '{"enabled":false,"min":0}'),
  stockWriteOffs: JSON.parse(localStorage.getItem('libreria_sh82_stock_writeoffs') || '[]'),
  queuedOrders: JSON.parse(localStorage.getItem('libreria_sh82_queued_orders') || '[]'),
  removedPeopleIds: JSON.parse(localStorage.getItem('libreria_sh82_removed_people_ids') || '[]'),
  userSalesModes: JSON.parse(localStorage.getItem('libreria_sh82_user_sales_modes') || '{}'),
  touchUiConfigByUser: JSON.parse(localStorage.getItem('libreria_sh82_touch_ui_config_by_user') || '{}'),
  categoryImages: JSON.parse(localStorage.getItem('libreria_sh82_category_images') || '{}'),
  orderCounters: JSON.parse(localStorage.getItem('libreria_sh82_order_counters') || '{}'),
  deletedRecordIds: JSON.parse(localStorage.getItem('libreria_sh82_deleted_record_ids') || '{"cashClosings":[],"sales":[],"products":[],"categories":[]}'),
  subcategories: JSON.parse(localStorage.getItem('libreria_sh82_subcategories') || '{}'),
  generalCash: JSON.parse(localStorage.getItem('libreria_sh82_general_cash') || '{"efectivo":0,"qr":0,"estado":"CERRADA","openedAt":"","closedAt":"","openedBy":"","closedBy":"","updatedAt":0}'),
  generalClosings: JSON.parse(localStorage.getItem('libreria_sh82_general_closings') || '[]'),
  cloudModuleMeta: JSON.parse(localStorage.getItem('libreria_sh82_cloud_module_meta') || '{}'),
  moduleHydration: JSON.parse(localStorage.getItem('libreria_sh82_module_hydration') || '{}')
};
...
const IMAGE_DB_NAME = 'libreria_sh82_images_db';
...
state.currentCart = [];
state.outflows = JSON.parse(localStorage.getItem('libreria_sh82_outflows') || '[]');
state.comboDraft = [];
state.activeDebtorId = '';
state.debtPayments = JSON.parse(localStorage.getItem('libreria_sh82_debt_payments') || '[]');
state.comboBuilderItems = [];
state.lastSyncAt = Number(localStorage.getItem('libreria_sh82_last_sync_at') || '0');
state.forceLogoutAt = Number(localStorage.getItem('libreria_sh82_force_logout_at') || '0');
state.cashBoxes = JSON.parse(localStorage.getItem('libreria_sh82_cash_boxes') || '[]');
state.selectedClosingIds = [];
state.generatedClosingsStats = null;
state.components = JSON.parse(localStorage.getItem('libreria_sh82_components') || '[]');
state.componentLinks = JSON.parse(localStorage.getItem('libreria_sh82_component_links') || '{}');
state.componentMoves = JSON.parse(localStorage.getItem('libreria_sh82_component_moves') || '[]');
...
state.activeCashBoxId = localStorage.getItem('libreria_sh82_active_cash_box_id') || '';
state.systemStatus = localStorage.getItem('libreria_sh82_system_status') || 'CAJA_CERRADA';
state.salesHistoryMode = 'all';

const SHARED_DB_PATH = 'libreria_sh82_shared';
const LEGACY_DB_PATH = 'libreria_sh82_legacy';
...
const defaultCloudConfig = {
  ...
  firebaseStorageBucket: 'libreria-sh.firebasestorage.app',
};
...
function normalizeCloudSeedObject(raw) {
  ...
  if (!merged.users.find((u) => u?.username === 'admin')) {
    merged.users.push({ username: 'admin', password: '54321', permissions: defaultPermissions(), createdBy: 'admin', enabled: true, lastActivityAt: Date.now(), lastLogoutAt: 0 });
  }
  return merged;
}
  ...
  for (const includeToken of authModes) {
    ...
    try {
      ...
      if (!hasModules) {
        ...
        console.info('[seed] Estructura modular creada en libreria_sh82_shared.', { modeLabel });
      }
...
function saveLocalState() {
  safeLocalSet('libreria_sh82_last_sync_at', String(state.lastSyncAt || 0));
  safeLocalSet('libreria_sh82_force_logout_at', String(state.forceLogoutAt || 0));
  safeLocalSet('libreria_sh82_cash_boxes', JSON.stringify(state.cashBoxes || []));
  safeLocalSet('libreria_sh82_active_cash_box_id', state.activeCashBoxId || '');
  safeLocalSet('libreria_sh82_system_status', state.systemStatus || 'CAJA_CERRADA');
  safeLocalSet('libreria_sh82_products', JSON.stringify(productsForLocalPersistence()));
  safeLocalSet('libreria_sh82_sales', JSON.stringify(state.sales));
  safeLocalSet('libreria_sh82_deleted_sales', JSON.stringify(state.deletedSales));
  safeLocalSet('libreria_sh82_cash_closings', JSON.stringify(state.cashClosings));
  safeLocalSet('libreria_sh82_cash_session', JSON.stringify(state.cashSession));
  safeLocalSet('libreria_sh82_users', JSON.stringify(state.users));
  safeLocalSet('libreria_sh82_current_user', JSON.stringify(state.currentUser));
  safeLocalSet('libreria_sh82_settings', JSON.stringify(state.settings));
  safeLocalSet('libreria_sh82_categories', JSON.stringify(state.categories));
  safeLocalSet('libreria_sh82_people', JSON.stringify(state.people));
  safeLocalSet('libreria_sh82_stock_config', JSON.stringify(state.stockConfig));
  safeLocalSet('libreria_sh82_stock_writeoffs', JSON.stringify(state.stockWriteOffs || []));
  safeLocalSet('libreria_sh82_outflows', JSON.stringify(state.outflows));
  safeLocalSet('libreria_sh82_debt_payments', JSON.stringify(state.debtPayments));
  safeLocalSet('libreria_sh82_components', JSON.stringify(state.components || []));
  safeLocalSet('libreria_sh82_component_links', JSON.stringify(state.componentLinks || {}));
  safeLocalSet('libreria_sh82_component_moves', JSON.stringify(state.componentMoves || []));
  safeLocalSet('libreria_sh82_queued_orders', JSON.stringify(state.queuedOrders || []));
  safeLocalSet('libreria_sh82_removed_people_ids', JSON.stringify(state.removedPeopleIds || []));
  safeLocalSet('libreria_sh82_user_sales_modes', JSON.stringify(state.userSalesModes || {}));
  safeLocalSet('libreria_sh82_touch_ui_config_by_user', JSON.stringify(state.touchUiConfigByUser || {}));
  safeLocalSet('libreria_sh82_category_images', JSON.stringify(categoryImagesForLocalPersistence()));
  safeLocalSet('libreria_sh82_order_counters', JSON.stringify(state.orderCounters || {}));
  safeLocalSet('libreria_sh82_deleted_record_ids', JSON.stringify(state.deletedRecordIds || { cashClosings: [], sales: [], products: [], categories: [] }));
  safeLocalSet('libreria_sh82_subcategories', JSON.stringify(state.subcategories || {}));
  safeLocalSet('libreria_sh82_general_cash', JSON.stringify(state.generalCash || { efectivo: 0, qr: 0, estado: 'CERRADA', openedAt: '', closedAt: '', openedBy: '', closedBy: '', updatedAt: 0 }));
  safeLocalSet('libreria_sh82_general_closings', JSON.stringify(state.generalClosings || []));
  safeLocalSet('libreria_sh82_cloud_module_meta', JSON.stringify(state.cloudModuleMeta || {}));
  safeLocalSet('libreria_sh82_module_hydration', JSON.stringify(state.moduleHydration || {}));
}
...
function ensureUsers() {
  if (!Array.isArray(state.users) || state.users.length === 0) {
    state.users = [{ username: 'admin', password: '54321', permissions: defaultPermissions(), createdBy: 'admin', enabled: true, lastActivityAt: Date.now(), lastLogoutAt: 0 }];
  }
  if (!state.users.find((u) => u.username === 'admin')) {
    state.users.push({ username: 'admin', password: '54321', permissions: defaultPermissions(), createdBy: 'admin', enabled: true, lastActivityAt: Date.now(), lastLogoutAt: 0 });
  }
...
function applySettings() {
  businessName && (businessName.textContent = state.settings.title1 || 'Librería SH82');
  homeSubtitle && (homeSubtitle.textContent = state.settings.title2 || 'Pantalla principal');
  posTitle && (posTitle.textContent = `Ventas - ${state.settings.title1 || 'Librería SH82'}`);
...
async function downloadClosingPdf(closingId) {
  ...
  try {
    ...
    const y0 = 12;
    doc.setFontSize(14);
    doc.text((state.settings?.title1 || 'Librería SH82'), 14, y0);
    doc.setFontSize(10);
...
async function downloadClosingsStatsPdf() {
  ...
  try {
    ...
    doc.setFontSize(14);
    doc.text((state.settings?.title1 || 'Librería SH82'), 14, 12);
    doc.setFontSize(10);
...
function billingPreviewConfig() {
  ...
  return {
    ...
    title: String(billingTitleInput?.value || state.settings?.title1 || cfg.title || 'Librería SH82'),
...
function renderBillingPreview() {
  ...
  body.innerHTML = `<div style="max-width:360px;margin:0 auto;padding:${marginPx}px;border:1px dashed #9aa7b5;background:#fff;color:#1b2430;font-family:monospace;">
    ...
    <div style="text-align:center;font-size:${Math.max(11, Number(cfg.titleSizePt || 12))}pt;font-weight:${cfg.titleBold ? 700 : 400};margin-bottom:8px;">${escapeHtml(state.settings?.title1 || cfg.title || 'Librería SH82')}</div>
...
async function handleLogin() {
  ...
  if (!user && username === 'admin' && password === '54321') {
    const legacyAdmin = state.users.find((u) => u.username === 'admin' && u.password === '5432');
    if (legacyAdmin) {
      legacyAdmin.password = '54321';
      user = legacyAdmin;
    }
  }
...
async function saveMainSettings() {
  ...
  state.settings.title1 = title1Input?.value?.trim() || 'Librería SH82';
  state.settings.title2 = title2Input?.value?.trim() || 'Pantalla principal';
  state.settings.posTitle = posTitleInput?.value?.trim() || 'POS Librería SH82';
...
async function saveBillingSettings() {
  ...
  billing.title = String(billingTitleInput?.value || billing.title || 'LIBRERÍA SH82').trim() || 'LIBRERÍA SH82';
...
async function bootstrap() {
  ...
  window.addEventListener('storage', (e) => {
    if (!e.key || !e.key.startsWith('libreria_sh82_') || document.hidden) return;
